...
func (t *SqliteConn) UserUpdate(user *record.User) error {
	if cmd_user_update == "" {
		cmd_user_update = fmt.Sprintf(`UPDATE %s
           ...
           WHERE %s = ? `,
			record.USER_STORE_NAME,
			FIELD_FULLNAME, FIELD_EMAIL,
			FIELD_LOGINNAME, FIELD_PASSWORD,
			FIELD_TOKEN, FIELD_LOGIN_DT,
			FIELD_LOGOUT_DT, FIELD_LASTAUTH_DT,
			FIELD_LASTFAILED_DT, FIELD_FAILCOUNT,
			FIELD_MAX_SESSION_DT, FIELD_TIMEOUT_DT,
			FIELD_UPDATED_DT, FIELD_DELETED_DT,
			FIELD_ISACTIVE,
			FIELD_ISLOGGEDIN,
			FIELD_GUID,
		)
